...
MUDANCAS:
✓ Usa analyze_usage_new.py (com UserClassificationEngine + SOLID)
✓ Usa license_optimizer_new.py (com LicenseOptimizer + SOLID)
...
PIPELINE_STEPS = [
    ...
    {
        "name": "9. [FASE 3] Analisando Histórico de Uso - SOLID (analyze_usage_new.py)",
        "cmd": [sys.executable, str(ROOT / "src" / "analyze_usage_new.py")],
        "skippable": False,
        "features": "✓ Dados REAIS (não mocks) ✓ UserClassificationEngine (SOLID) ✓ 6 rules independentes"
    },
    {
        "name": "10. [FASE 3-B] Detector de Otimização - SOLID (license_optimizer_new.py)",
        "cmd": [sys.executable, str(ROOT / "src" / "license_optimizer_new.py")],
        "skippable": False,
