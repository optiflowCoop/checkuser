...
if __name__ == '__main__':
    # Script executado diretamente - processa arquivos de AD
    pass
else:
    ...
    IN_DISABLED_FILE = ROOT / 'adUsers' / 'adUsersdesabilitadas.csv'
    ...
    def _normalize(text):
        """Normaliza texto para comparação (lowercase, trim)."""
    ...
    def main():
        ...
        print(f"📥 Lendo dados do AD habilitados: {IN_FILE.name}")
        ad_rows = parse_ad_csv(IN_FILE)

        disabled_rows = []
        if IN_DISABLED_FILE.exists():
            print(f"📥 Lendo dados do AD desabilitados: {IN_DISABLED_FILE.name}")
            disabled_rows = parse_ad_csv(IN_DISABLED_FILE)
        else:
            print(f"⚠️ Arquivo de AD desabilitados não encontrado: {IN_DISABLED_FILE}")

        print(f"   Total de registros habilitados lidos: {len(ad_rows)}")
        if ad_rows:
            print(f"   Colunas (habilitados): {list(ad_rows[0].keys())}")
        print(f"   Total de registros desabilitados lidos: {len(disabled_rows)}")

        # Construir conjuntos de referência dos desabilitados por email e DisplayName
        disabled_emails = {_normalize(r.get('mail')) for r in disabled_rows if r.get('mail')}
        disabled_names = {_normalize(r.get('DisplayName')) for r in disabled_rows if r.get('DisplayName')}

        # Filtrar habilitados removendo qualquer registro que apareça na lista de desabilitados
        filtered_ad_rows = []
        for r in ad_rows:
            email = _normalize(r.get('mail'))
            name = _normalize(r.get('DisplayName'))
            enabled_flag = r.get('Enabled', '').strip().lower() == 'true'

            # Se estiver na lista de desabilitados por email ou nome, descartar
            if email and email in disabled_emails:
                continue
            if not email and name and name in disabled_names:
                continue

            # Mantém apenas registros marcados como habilitados
            if enabled_flag:
                filtered_ad_rows.append(r)

        # Estatísticas após filtro
        total = len(filtered_ad_rows)
        enabled = sum(1 for r in filtered_ad_rows if r.get('Enabled', '').strip().lower() == 'true')
        disabled = len(disabled_rows)

        # Contagem por domínio
        domain_counts = {}
        for r in filtered_ad_rows:
            email = r.get('mail', '')
            domain = extract_domain(email)
            domain_counts[domain] = domain_counts.get(domain, 0) + 1

        foresea_count = sum(1 for r in filtered_ad_rows if 'foresea.com' in str(r.get('mail', '')).lower())
        partner_count = sum(1 for r in filtered_ad_rows if 'foresea-partner.com' in str(r.get('mail', '')).lower())

        print(f"\n📊 Estatísticas do AD (após saneamento):")
        print(f"   Total de usuários habilitados: {total}")
        print(f"   Contas habilitadas (Enabled=TRUE): {enabled}")
        print(f"   Contas desabilitadas (arquivo desabilitados): {disabled}")
        ...
        print(f"\n   Top 10 domínios (habilitados):")
        for domain, count in sorted(domain_counts.items(), key=lambda x: -x[1])[:10]:
            print(f"      {domain}: {count}")

        # Padronizar e escrever arquivo consolidado apenas com habilitados saneados
        ...
        print(f"   {total} usuários habilitados exportados com {len(fieldnames)} colunas")
