...
def build_html_structure(summary, governance, app_points, domains):
    # --- Analytics de AppPoints e Cenários para o Simulador JS ---
    inativos_count = 0
    downgrade_count = 0
    concurrent_count = 0
    ...
    for u in app_points:
        ...
        # 1. As-Is (Migração Cega sem cortes)
        if is_prem:
            if is_auth:
        ...
        # 2. Saneado (Corta os Inativos >90d, mas não aplica otimização de modelo)
        if is_prem:
            if is_auth:
        ...
        # 3. Otimizado (Aplica Downgrades e Fator Estatístico P95 nas Sondas)
    ...
    # Cálculos globais para os gráficos de Estresse (Cotidiano, Pico Seguro, Crise)
    custo_p50 = scenarios_data['otimizado']['pA'] * 5 + (
                scenarios_data['otimizado']['pC'] * (avg_f_p50 / avg_f_p95)) * 15 + scenarios_data['otimizado'][
                    'bA'] * 3 + (scenarios_data['otimizado']['bC'] * (avg_f_p50 / avg_f_p95)) * 10
    custo_p95 = scenarios_data['otimizado']['pA'] * 5 + scenarios_data['otimizado']['pC'] * 15 + \
                scenarios_data['otimizado']['bA'] * 3 + scenarios_data['otimizado']['bC'] * 10
    custo_p100 = scenarios_data['otimizado']['pA'] * 5 + (
                scenarios_data['otimizado']['pC'] * (avg_f_p100 / avg_f_p95)) * 15 + scenarios_data['otimizado'][
                     'bA'] * 3 + (scenarios_data['otimizado']['bC'] * (avg_f_p100 / avg_f_p95)) * 10

    # --- Tabelas de Governança (Restauradas) ---
...
<body>
    ...
    <div id="tab-gov" class="container tab-content">
        ...
        <div class="card">
            ...
            <div class="type-analysis-grid">{''.join(title_divergence_html)}</div>
        </div>

        <div class="card">
            <h2 class="card-header">Conflitos de Multi-Ambiente (Cross-Env)</h2>
            {render_table(['USERID', 'Bases Encontradas', 'Nomes de Exibição', 'Conclusão'], cross_env_rows, 'table-cross-env', 'gov-table')}
        </div>

        <div class="card">
            <h2 class="card-header">Colisões de Active Directory (LOGINID)</h2>
            {render_table(['LOGINID AD', 'Bases', 'USERIDs', 'Nomes Cadastrados'], login_conflicts_rows, 'table-login-conflicts', 'gov-table')}
        </div>
        ...
        <div class="card">
            ...
            {render_table(['ID Bruto', 'Nome', 'Hipótese Sistêmica', 'Decisão / Ação'], worklist_rows, 'table-worklist', 'gov-table')}
        </div>
    </div>
