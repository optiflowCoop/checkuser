...
def classify_user_tier(app_list, login_count_90d, operational_presence, title=''):
    ...
    if operational_presence == 'OFFSHORE':
        if is_critical:
            # Funções críticas offshore precisam AUTHORIZED (ex: almoxarife sempre disponível)
            if has_premium:
                return ('CRITICAL_OFFSHORE_OG', 'PREMIUM_AUTHORIZED', 5)
            else:
                return ('CRITICAL_OFFSHORE_STD', 'BASE_AUTHORIZED', 2)
        else:
            # Funções operacionais offshore = CONCURRENT (revezamento turno)
            if has_premium:
    ...
    if has_premium:
        # Premium O&G: só Authorized se uso muito intenso (>60 logins/90d = 3-4x/semana)
        if login_count_90d > 60:
            return ('POWER_OG', 'PREMIUM_AUTHORIZED', 5)
        else:
            return ('MEDIUM_OG', 'PREMIUM_CONCURRENT', 15)
    else:
        # Base Standard: só Authorized se uso muito intenso
        if login_count_90d > 60:
...
def main():
    ...
    print(f"✓ Carregados {len(email_map)} emails")
    ...
    if not logintrack_path.exists():
        ...
        print("Execute: python run_pipeline.py (sem --skip-extract)")
        return
    ...
    for identity in identities:
        ...
        # Focar apenas em ACTIVE
        if status != 'ACTIVE':
            continue
        
        # Buscar dados de acesso (groups, title, persongroup)
        ...
        # Buscar uso real de cada ambiente
        total_logins = 0
        all_apps = set()
        ...
        output_rows.append({
            ...
            'EMAIL': email,
            'USER_CATEGORY': user_category,  # FORESEA ou TEMPORARY
            'OPERATIONAL_PRESENCE': operational_presence,  # OFFSHORE ou ONSHORE
            'STATUS': status,
            'TITLE': title,
            ...
            'PREMIUM_APPS': '; '.join(sorted(premium_apps)[:5]),  # Top 5
            'STANDARD_APPS': '; '.join(sorted(standard_apps)[:5]),  # Top 5
            'USER_TIER': tier,
    ...
    if output_rows:
        ...
        total_cost = sum(r['APP_POINTS_COST'] for r in output_rows)
